...
let SurveysService = class SurveysService {
    constructor(dataSource, surveysRepository, routesRepository, questionsRepository, optionsRepository, responsesRepository, answersRepository) {
    ...
    async findPublicBySlug(publicSlug) {
        const survey = await this.surveysRepository.findOne({
    ...
    async submitPublicResponse(publicSlug, dto, meta) {
        const survey = await this.surveysRepository.findOne({
            where: { publicSlug, isActive: true },
            relations: {
                questions: { options: true },
    ...
    async saveQuestions(manager, surveyId, questions) {
        const questionRepo = manager.getRepository(SurveyQuestion);
        const optionRepo = manager.getRepository(QuestionOption);
        for (const questionDto of questions) {
            ...
            const shouldHaveOptions = questionDto.tipo === QuestionType.SINGLE_CHOICE ||
                questionDto.tipo === QuestionType.MULTIPLE_CHOICE;
            if (shouldHaveOptions && questionDto.options && questionDto.options.length > 0) {
                const options = questionDto.options.map((optionDto) => optionRepo.create({
...
SurveysService = __decorate([
    ...
    __param(1, InjectRepository(Survey)),
    __param(2, InjectRepository(UniversityRoute)),
    __param(3, InjectRepository(SurveyQuestion)),
    __param(4, InjectRepository(QuestionOption)),
    __param(5, InjectRepository(SurveyResponse)),
    __param(6, InjectRepository(ResponseAnswer)),
